...
export default function GroupMembers() {
  ...
  async function togglePermission(
    ...
    try {
      ...
      setMembers((ms) =>
        ms.map((m) => (m.address === memberAddr ? { ...m, perms: newMask } : m))
      );
  ...
  async function handleAdd(e: FormEvent) {
    ...
    if (
      members.some((m) => m.address.toLowerCase() === newAddr.toLowerCase())
    ) {
  ...
  if (loading) {
    return (
      <p style={{ textAlign: "center", marginTop: "2rem" }}>Loading members…</p>
    );
  }
